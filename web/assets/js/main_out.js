(function() {
    ...
    */
    ...
    function rotateBufferBytes(buffer, key) {
        const oldBuffer = new DataView(buffer);
        const newBuffer = new DataView(new ArrayBuffer(oldBuffer.byteLength));
        for(let i = 0; i < newBuffer.byteLength; i++) newBuffer.setUint8(i, oldBuffer.getUint8(i) ^ key >>> (i % 4 * 8) & 255);
    ...
    const SKIN_URL = 'http://localhost:3000/skins/';
    ...
    const SEND_254 = new Uint8Array([254, 23, 0, 0, 0]);
    const SEND_255 = new Uint8Array([255, 137, 121, 0, 0, 0, 0, 0, 0]);
    const UINT8_CACHE = {
        1: new Uint8Array([1]),
    ...
    function wsSend(data) {
        if(ws.readyState === WebSocket.OPEN) {
            if(encryptionKey) {
                data = rotateBufferBytes(data.buffer, encryptionKey);
                encryptionKey = rotateEncryptionKey(encryptionKey);
            };
            ws.send(data);
        };
    }
    ...
    function wsMessage(data) {
        ...
        if(decryptionKey) {
            handleBuffer(rotateBufferBytes(data.data, decryptionKey ^ 31113));
        } else {
            handleBuffer(data.data);
        };
    }

    function handleBuffer(buffer) {
        const reader = new Reader(new DataView(buffer), 0, true);
        switch(reader.getUint8()) {
            ...
            case 32: {
                const cell = reader.getUint32();
                cells.mine.push(cell);
                break;
            };
            case 53: {
                leaderboard.items = [];
                ...
                while (reader._o < reader.view.byteLength && count < 10) {
                    let lbName, 
                    isMe;
                    const flagMask = reader.getUint8();
                    if(flagMask & 2) {
                        lbName = reader.getStringUTF8();
                    };
                    if(flagMask & 4) reader._o += 4;
                    if(flagMask & 8) {
                        lbName = settings.nick;
                        isMe = true;
                    };
            ...
            case 241: {
                url = ws.url.replace(/(wss:\/\/)/, "").toString().replace(/[?]party_id=(\w+)/, "").toString().replace(/(\/)/, "");
                decryptionKey = reader.getUint32();
                encryptionKey = murmur2(`${url}${reader.getStringUTF8()}`, 255)
                break;
            };
            case 242: {
                mouseInterval = setInterval(() => {
                    sendMouseMove((mouseX - mainCanvas.width / 2) / camera.scale + camera.x, (mouseY - mainCanvas.height / 2) / camera.scale + camera.y);
                }, 40);
                break;
            };
            case 255: {
                handleCompressedBuffer(uncompressBuffer(new Uint8Array(reader.view.buffer.slice(5)), new Uint8Array(reader.getUint32())));
                break;
            };
        };
    };

    function handleCompressedBuffer(buffer) {
        const reader = new Reader(new DataView(buffer.buffer), 0, true);
        switch(reader.getUint8()) {
            case 16: {
                ...
                while (true) {
                    ...
                    const flagMask = reader.getUint8();
                    const extendedFlagMask = flagMask & 128 ? reader.getUint8() : 0;

                    const flags = {
                        jagged: !!(flagMask & 1),
                        isPellet: !!(extendedFlagMask & 1),
                    };

                    if(flagMask & 2) color = new Color(reader.getUint8(), reader.getUint8(), reader.getUint8());
                    if(flagMask & 4) skin = reader.getStringUTF8();
                    if(flagMask & 8) name = reader.getStringUTF8();
                    if(extendedFlagMask & 4) reader._o += 4;
                ...
                for(const id of cells.mine) {
                    const cell = cells.byId.get(id);
                    cell.setName(settings.nick);
                    cell.setSkin(settings.skin);
                };
            ...
            case 64: { // set border
                ...
                if (buffer.buffer.byteLength === 33) break;
                if (!mapCenterSet) {
                    mapCenterSet = true;
    ...
    function sendMouseMove (x, y) {
        ...
        writer.setInt32(x);
        writer.setInt32(y);
        writer.setInt32(decryptionKey);
        wsSend(new Uint8Array(writer._b));
    }
    ...
    function buildGallery() {
        ...
        for (i = 0; i < knownSkins.length; i++) {
            ...
            c += `<img class="circular" src="http://localhost:3000/skins/${knownSkins[i].skinName}.png">`;
    ...
    class Cell {
        ...
        updateNumPoints() {
            let numPoints = Math.min(Math.max(this.s * camera.scale | 0, 120), this.s);
